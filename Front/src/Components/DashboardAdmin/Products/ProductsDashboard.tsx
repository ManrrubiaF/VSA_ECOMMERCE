...
export default function ProductsDashBoard() {
  ...
  const { elementsPerPage, currentPage } = useAppSelector(
  ...
  return (
    <div className={Styles.divMayor}>
      ...
      {productsOnPage &&
        productsOnPage.map((oneProduct) => (
          <div className={Styles.dataProduct} key={oneProduct.id}>
            <div
              onClick={() => navigate(`/someplace/detail/${oneProduct.id}`)}
            ...
            <div>
              {oneProduct.active ? <p> Activo </p> : <p> Desactivado </p>}
            </div>
          </div>
        ))}
      <Toaster />
